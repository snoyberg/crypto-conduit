...
getType :: Monad m => m output -> output
getType = undefined
...
-- creates a digest @d@.
sinkHash :: C.Hash ctx d => MonadSink B.ByteString m d
sinkHash =
    self
  where
...
getBlock :: BlockMode -> C.ByteLength -> MonadSink B.ByteString m Block
...
conduitEncryptEcb :: (Monad m, C.BlockCipher k) =>
                     k -- ^ Cipher key.
                  -> MonadConduit B.ByteString m B.ByteString
conduitEncryptEcb k =
...
sinkCbcMac :: (Monad m, C.BlockCipher k) =>
              k -- ^ Cipher key.
           -> MonadSink B.ByteString m B.ByteString
sinkCbcMac k =
      ...
      go iv = do
          ...
          case x of
              ...
              LastOne input
                  ...
                  | otherwise -> liftStreamMonad $ fail "sinkCbcMac: input has an incomplete final block."
...
blocked :: Monad m =>
        ...
        -> MonadConduit B.ByteString m Block
      ...
      go state = do
          ...
          case x of
              ...
              LastOne input
                  ...
                  | otherwise -> liftStreamMonad (final state input) >>= yield >> finish
